...
function RideSummaryModal({ ride, onDone, completing }: RideSummaryModalProps) {
  ...
  const fareDisplay = formatCurrency(ride.price_offer, ride.currency);
...
function DriverView() {
  ...
  const handleAccept = async (id: string) => {
    ...
    try {
      ...
      console.error('[DriverView] Accept failed:', e);
    }
  };

  const handleBargain = (id: string, price: number, currency: string) => {
    console.log('[DriverView] Opening bargain for ride:', id, 'price:', price);
  ...
  const submitBargain = async (pct: 10 | 25 | 50) => {
    ...
    try {
      ...
      console.error('[DriverView] Bargain failed:', e);
    }
  };
  ...
  const handleIgnore = async (id: string) => {
    ...
    try {
      ...
      console.error('[DriverView] Reject failed:', e);
    }
  };
    ...
    <View style={{ flex: 1, backgroundColor: COLORS.background }}>
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <Text style={{ fontSize: 13, color: isAvailable ? COLORS.success : COLORS.danger, fontFamily: 'Nunito_600SemiBold' }}>
            {mutedLabel}
          </Text>
          <Switch
            value={isAvailable}
      ...
      {!isAvailable && (
        ...
        >
          <Text style={{ fontSize: 13, color: COLORS.danger, fontFamily: 'Nunito_600SemiBold', textAlign: 'center' }}>
            Ride requests muted — toggle to receive requests
          </Text>
        </View>
      )}
      ...
      >
        {loading ? (
          ...
          <View style={{ alignItems: 'center', paddingTop: 60, gap: 12 }}>
            ...
            <Text style={{ fontSize: 17, fontWeight: '600', color: COLORS.text, fontFamily: 'Nunito_600SemiBold' }}>
              {t('noRideRequests')}
            </Text>
      ...
      <Modal visible={!!acceptModal} transparent animationType="fade">
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 24 }}>
          <View style={{ backgroundColor: COLORS.surface, borderRadius: 20, padding: 24, width: '100%', gap: 16 }}>
            ...
            <Text style={{ fontSize: 22, fontWeight: '700', color: COLORS.primary, fontFamily: 'Nunito_700Bold' }} selectable>
              {acceptModal?.riderPhone || 'N/A'}
            </Text>
            <AnimatedPressable
              onPress={() => {
                const phone = acceptModal?.riderPhone;
                if (phone) {
                  console.log('[DriverView] Calling rider:', phone);
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: COLORS.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: insets.bottom + 24, gap: 16 }}>
            ...
            <Text style={{ fontSize: 14, color: COLORS.textSecondary, fontFamily: 'Nunito_400Regular' }}>
              Current offer: {bargainModal ? formatCurrency(bargainModal.price, bargainModal.currency) : ''}
            </Text>
            {([10, 25, 50] as const).map((pct) => {
              ...
              return (
                ...
                >
                  ...
                  <Text style={{ fontSize: 15, fontWeight: '600', color: COLORS.textSecondary, fontFamily: 'Nunito_600SemiBold' }}>
                    {newPriceDisplay}
                  </Text>
                </AnimatedPressable>
              );
...
function RiderView() {
  ...
  const pollRide = useCallback(async (rideId: string) => {
    ...
    try {
      ...
      console.error('[RiderView] Poll failed:', e);
    }
  }, []);
  ...
  const handleConfirmRequest = async () => {
    ...
    try {
      const res = await apiPost<ActiveRide>('/api/rides', {
      ...
      console.error('[RiderView] Create ride failed:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBargainRespond = async (accept: boolean) => {
    ...
    console.log('[RiderView] Bargain respond:', accept ? 'accept' : 'reject', 'for ride:', activeRide.id);
    try {
      await apiPost(`/api/rides/${activeRide.id}/bargain/respond`, { accept });
      setBargainModal(false);
      await pollRide(activeRide.id);
    } catch (e) {
      console.error('[RiderView] Bargain respond failed:', e);
    }
  };
      ...
      <View style={{ flex: 1, backgroundColor: COLORS.background }}>
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 120 }}>
          ...
          >
            ...
            <Text style={{ fontSize: 18, fontWeight: '700', color: COLORS.textSecondary, fontFamily: 'Nunito_700Bold' }}>
              {priceDisplay}
            </Text>
            ...
            {isAccepted && (
              <View style={{ backgroundColor: COLORS.successMuted, borderRadius: 12, padding: 14, alignItems: 'center', gap: 4 }}>
                ...
                <Text style={{ fontSize: 16, fontWeight: '700', color: COLORS.success, fontFamily: 'Nunito_700Bold' }}>
                  {t('driverOnTheWay')}
                </Text>
                {activeRide.driver_first_name ? (
                  <Text style={{ fontSize: 14, color: COLORS.textSecondary, fontFamily: 'Nunito_400Regular' }}>
                    Driver: {activeRide.driver_first_name}
                  </Text>
                ) : null}
              </View>
            )}
            ...
            {isCancelled && (
              <View style={{ backgroundColor: COLORS.dangerMuted, borderRadius: 12, padding: 14, alignItems: 'center', gap: 8 }}>
                <Text style={{ fontSize: 15, fontWeight: '600', color: COLORS.danger, fontFamily: 'Nunito_600SemiBold', textAlign: 'center' }}>
                  {t('requestCancelled')}
                </Text>
                <AnimatedPressable
            ...
            {isCompleted && (
              <View style={{ backgroundColor: COLORS.successMuted, borderRadius: 12, padding: 14, alignItems: 'center', gap: 8 }}>
                <Text style={{ fontSize: 22 }}>✅</Text>
                <Text style={{ fontSize: 15, fontWeight: '600', color: COLORS.success, fontFamily: 'Nunito_600SemiBold' }}>
                  Ride completed!
                </Text>
                <AnimatedPressable
                  onPress={handleNewRequest}
        ...
        <Modal visible={bargainModal} transparent animationType="slide">
          <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
            <View style={{ backgroundColor: COLORS.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: insets.bottom + 24, gap: 16 }}>
              ...
              <Text style={{ fontSize: 15, color: COLORS.textSecondary, fontFamily: 'Nunito_400Regular' }}>
                The driver has proposed a new price:
              </Text>
              <Text style={{ fontSize: 26, fontWeight: '800', color: COLORS.primary, fontFamily: 'Nunito_800ExtraBold' }}>
                {bargainPriceDisplay || priceDisplay}
              </Text>
              <View style={{ flexDirection: 'row', gap: 12 }}>
                <AnimatedPressable
                  onPress={() => handleBargainRespond(true)}
                  style={{
                    flex: 1,
                    backgroundColor: COLORS.success,
                    borderRadius: 14,
                ...
                <AnimatedPressable
                  onPress={() => handleBargainRespond(false)}
                  style={{
                    flex: 1,
                    borderWidth: 1.5,
    ...
    <View style={{ flex: 1, backgroundColor: COLORS.background }}>
      ...
      >
        <View style={{ paddingTop: insets.top + 12, marginBottom: 24 }}>
          ...
          <Text style={{ fontSize: 14, color: COLORS.textSecondary, fontFamily: 'Nunito_400Regular', marginTop: 4 }}>
        ...
        <View style={{ marginBottom: 16 }}>
          ...
          >
            ...
            <TextInput
              ...
              style={{ flex: 1, fontSize: 15, color: COLORS.text, fontFamily: 'Nunito_400Regular' }}
            />
          </View>
        </View>
        ...
        <View style={{ marginBottom: 28 }}>
          ...
          >
            ...
            >
              <Text style={{ fontSize: 13, fontWeight: '700', color: COLORS.textSecondary, fontFamily: 'Nunito_700Bold' }}>
                {currency}
              </Text>
            </View>
            <TextInput
        ...
        <AnimatedPressable
          ...
          style={{
            ...
            alignItems: 'center',
            justifyContent: 'center',
            boxShadow: canSubmit ? '0 4px 16px rgba(245,197,24,0.35)' : undefined,
          }}
        >
          {loading ? (
